...
def is_convergent(
    ...
    basis converges fast enough that at the right ℏ (around -0.7 at
    M = 3) the error drops below 0.01.
    """
...
def _hbar_sweep_grid() -> list[sp.Expr]:
    """A grid of negative ℏ values; the convergent region is around -0.5 to -0.8."""
    return [sp.Rational(k, 10) for k in range(-15, 1)]


def analyze(solution: HamSolution) -> dict[str, sp.Expr | bool]:
    """Diagnostics for an exponential-basis Blasius solution.

    Sweeps a grid of ℏ and reports the value minimising
    `|f''(0) - Howarth|`. The optimal ℏ shifts with working order
    (a general HAM feature, more pronounced for slow-converging
    bases). Callers wanting just one number for a fixed problem
    should pick ℏ by hand from the grid that gives the smallest
    error at their working order.
    """
    ...
    return {
        ...
        "abs_error_at_best_hbar": sp.Abs(fdd_at_grid[best_hbar] - HOWARTH_F_DOUBLE_PRIME_AT_ZERO),
        "convergent_at_best_hbar": is_convergent(solution, best_hbar),
    }


if __name__ == "__main__":  # pragma: no cover -- runnable script entry point
    print("Exponential-basis Blasius — convergence of f''(0)")
    print(f"  Howarth reference: f''(0) ≈ {float(HOWARTH_F_DOUBLE_PRIME_AT_ZERO):.4f}")
    print(f"  alpha (fixed): {ALPHA}")
    print()
    print("Best ℏ in [-1.5, 0] (in steps of 1/10) at each working order:")
    print(f"  {'M':>2}  {'best ℏ':>8}  {'f(0)':>10}  {'|error|':>9}  {'gate':>6}")
    for m in range(1, 5):
        sol = solve_to(m)
        analysis = analyze(sol)
        best_hbar = analysis["best_hbar_for_f_double_prime"]
        fdd = float(analysis["f_double_prime_at_zero_best"])
        err = float(analysis["abs_error_at_best_hbar"])
        gate = analysis["convergent_at_best_hbar"]
        print(f"  {m:>2}  {best_hbar!s:>8}  {fdd:+.5f}  {err:.5f}  {gate!s:>6}")
